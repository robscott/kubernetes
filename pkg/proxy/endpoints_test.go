...
func TestEndpointSliceUpdate(t *testing.T) {
	testCases := map[string]struct {
		...
		"add a simple slice that doesn't already exist": {
			startingSlices:        []*discovery.EndpointSlice{},
			endpointChangeTracker: NewEndpointChangeTracker("host1", nil, nil, nil),
			namespacedName:        types.NamespacedName{Name: "svc1", Namespace: "ns1"},
			paramEndpointSlice:    generateEndpointSlice("svc1", "ns1", 1, 3, 999, []string{"host1", "host2"}, []int32{80, 443}),
		...
		"add a slice that overlaps with existing state": {
			startingSlices: []*discovery.EndpointSlice{
				generateEndpointSlice("svc1", "ns1", 1, 3, 999, []string{"host1", "host2"}, []int32{80, 443}),
				generateEndpointSlice("svc1", "ns1", 2, 2, 999, []string{"host1", "host2"}, []int32{80, 443}),
			},
			endpointChangeTracker: NewEndpointChangeTracker("host1", nil, nil, nil),
			namespacedName:        types.NamespacedName{Name: "svc1", Namespace: "ns1"},
			paramEndpointSlice:    generateEndpointSlice("svc1", "ns1", 1, 5, 999, []string{"host1"}, []int32{80, 443}),
			paramRemoveSlice:      false,
			expectedReturnVal:     true,
			expectedCurrentChange: map[ServicePortName][]*BaseEndpointInfo{
				makeServicePortName("ns1", "svc1", "port-0"): {
					...
					&BaseEndpointInfo{Endpoint: "10.0.2.1:80", IsLocal: false},
				...
				makeServicePortName("ns1", "svc1", "port-1"): {
					...
					&BaseEndpointInfo{Endpoint: "10.0.2.1:443", IsLocal: false},
		...
		"add a slice that overlaps with existing state and partial ports": {
			...
			endpointChangeTracker: NewEndpointChangeTracker("host1", nil, nil, nil),
			namespacedName:        types.NamespacedName{Name: "svc1", Namespace: "ns1"},
			paramEndpointSlice:    generateEndpointSliceWithOffset("svc1", "ns1", 3, 1, 5, 999, []string{"host1"}, []int32{80}),
			paramRemoveSlice:      false,
			expectedReturnVal:     true,
			expectedCurrentChange: map[ServicePortName][]*BaseEndpointInfo{
				makeServicePortName("ns1", "svc1", "port-0"): {
		...
		"remove a slice that doesn't even exist in current state": {
			startingSlices: []*discovery.EndpointSlice{
				generateEndpointSlice("svc1", "ns1", 1, 5, 999, []string{"host1"}, []int32{80, 443}),
				generateEndpointSlice("svc1", "ns1", 2, 2, 999, []string{"host1"}, []int32{80, 443}),
			},
			endpointChangeTracker: NewEndpointChangeTracker("host1", nil, nil, nil),
			namespacedName:        types.NamespacedName{Name: "svc1", Namespace: "ns1"},
			paramEndpointSlice:    generateEndpointSlice("svc1", "ns1", 3, 5, 999, []string{"host1"}, []int32{80, 443}),
			paramRemoveSlice:      true,
			expectedReturnVal:     false,
			expectedCurrentChange: nil,
		},
		...
		"transition all endpoints to unready state": {
			...
			endpointChangeTracker: NewEndpointChangeTracker("host1", nil, nil, nil),
			namespacedName:        types.NamespacedName{Name: "svc1", Namespace: "ns1"},
			paramEndpointSlice:    generateEndpointSlice("svc1", "ns1", 1, 3, 1, []string{"host1"}, []int32{80, 443}),
			paramRemoveSlice:      false,
			expectedReturnVal:     true,
			expectedCurrentChange: map[ServicePortName][]*BaseEndpointInfo{},
		},
		...
		"transition all endpoints to ready state": {
			startingSlices: []*discovery.EndpointSlice{
				generateEndpointSlice("svc1", "ns1", 1, 2, 1, []string{"host1", "host2"}, []int32{80, 443}),
			},
			endpointChangeTracker: NewEndpointChangeTracker("host1", nil, nil, nil),
			namespacedName:        types.NamespacedName{Name: "svc1", Namespace: "ns1"},
			paramEndpointSlice:    generateEndpointSlice("svc1", "ns1", 1, 2, 999, []string{"host1"}, []int32{80, 443}),
			paramRemoveSlice:      false,
			expectedReturnVal:     true,
			expectedCurrentChange: map[ServicePortName][]*BaseEndpointInfo{
				makeServicePortName("ns1", "svc1", "port-0"): {
		...
		"transition some endpoints to ready state": {
			startingSlices: []*discovery.EndpointSlice{
				generateEndpointSlice("svc1", "ns1", 1, 3, 2, []string{"host1"}, []int32{80, 443}),
				generateEndpointSlice("svc1", "ns1", 2, 2, 2, []string{"host1"}, []int32{80, 443}),
			},
			endpointChangeTracker: NewEndpointChangeTracker("host1", nil, nil, nil),
			namespacedName:        types.NamespacedName{Name: "svc1", Namespace: "ns1"},
			paramEndpointSlice:    generateEndpointSlice("svc1", "ns1", 1, 3, 3, []string{"host1"}, []int32{80, 443}),
			paramRemoveSlice:      false,
			expectedReturnVal:     true,
			expectedCurrentChange: map[ServicePortName][]*BaseEndpointInfo{
				makeServicePortName("ns1", "svc1", "port-0"): {
	...
	for name, tc := range testCases {
		tc.endpointChangeTracker.CacheEndpointSlices(tc.startingSlices)
		retVal := tc.endpointChangeTracker.EndpointSliceUpdate(tc.paramEndpointSlice, tc.paramRemoveSlice)
		assert.Equal(t, tc.expectedReturnVal, retVal, "[%s] EndpointSliceUpdate returned %v", name, retVal)
		assert.NotNil(t, tc.endpointChangeTracker.items, "Expected ect.items to not be nil")
		if tc.expectedCurrentChange == nil {
			assert.Nil(t, tc.endpointChangeTracker.items[tc.namespacedName], "Expected ect.items[namespacedName] to be nil")
		} else {
			assert.NotNil(t, tc.endpointChangeTracker.items[tc.namespacedName], "Expected ect.items[namespacedName] to not be nil")
